...
owl_single <-function(H, A, R2, pi, pentype='lasso', kernel='linear', sigma=c(0.03,0.05,0.07), clinear=2.^(-2:2), m=4, e=1e-5)  {
...
## a general K-stage outcome-weighted learning
owl <- function(H,AA,RR,n,K,pi='estimated', res.lasso=T, loss='hinge', kernel='linear', augment=T, sigma=c(0.03,0.05,0.07), c=2.^(-2:2), m=4, s=2.^(-2:2)) {
  ...
  if(pi[[1]][1]=='estimated' & K==1) {
      Y = AA*0.5 + 0.5
      pi_logit = cv.glmnet(H, Y, family = "binomial", nfold=3)
  ...
  for (j in K:1) {
    ...
    if(is.na(max(results[[j]]$treatment))) {
      has_error = 1
      break
    }
